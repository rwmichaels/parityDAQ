/*************************************************************************
 *
 *  vqwkLib.c   -  Driver library for readout of a TRIUMF VME QWEAK 8 
...
int  vqwkBlockRead(UINT16 index, UINT32 *data){
  ...
  /* R.M. Disabled until I get the sysVme* symbols */
  ...
  logMsg("vqwkBlockRead:  WARNING !!! Not working .... later \n",0,0,0,0,0,0);
...
/* Code may not be needed as vqwkRead does the same thing I think. */
void readStr7200(UINT16 index) {
    
    
    UINT32 data[48]; /* declare buffer that will hold all data for one adc. */
    UINT32 outword;
    int buffersize = 48;
    int cnt=0;
    int ii,jj;
    ...
    if (index<Nvqwk && vqwkp[index]!=NULL && vqwkp[index]!=&missingadc) {
    /*Good setup --taken from printStatus method in this doc-- */
	
	for(ii=0;ii<8;ii++) { 			/* run over all channels in the adc. */

	    int blknum = vqwkp[index]->num_blocks;		/* get the total number of blocks */
	    int dnum = blknum+2;				/* total number of data per channel */

	    for(jj=0;jj<blknum;jj++) {		/* run over all data stored in channel */
	   	data[cnt] = vqwkp[index]->adc[ii].block[jj];
		cnt++;
		if(cnt==buffersize) {
		    printf("\nERROR: BUFFER TO SMALL!!!  ---source: vqwk.c::readStr7200()\n\n");
		    exit(1);
		}
	    }

			/* get the total */
	    data[cnt] = vqwkp[index]->adc[ii].total; 		
	    cnt++;
	    if(cnt==buffersize) {
		printf("\nERROR: BUFFER TO SMALL!!!  ---source: vqwk.c::readStr7200()\n\n");
		exit(1);
	    ...
	    outword = (vqwkp[index]->adc[ii].nsamples << 16) + (vqwkp[index]->adc[ii].seqnum << 8) + 
		(vqwkp[index]->adc[ii].unused << 8); 		
	    data[cnt] = outword;
	    cnt++;
	    if(cnt==buffersize) {
		printf("\nERROR: BUFFER TO SMAll!!!  ---source: vqwk.c::readStr7200()\n\n");
	...
	printf("\nERROR: adc not found!!,  --source: vqwk.c::readStr7200()\n\n");
    }

    for(ii=0;ii<48;ii++)  /* for now just print buffer to screen*/
	printf("\n%d",data[ii]);	
...
void BobTest1(int nsamp) {

  /* Bob's test of Qweak ADC.  Adjustable Samples-per-block */
  ...
  for (i = 0; i<numvalues; i++) {
    printf("ADC[%d] = %d = 0x%x \n",i,adcdata[i],adcdata[i]);
  }

  /* flush the buffer */

  while (vqwkp[0]->data_ready != 0){
    numvalues = vqwkReadRegisters(0, 0xFF, adcdata);
    printf("Flushing buffers, num = %d\n",numvalues);
  }
