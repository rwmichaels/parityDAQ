...
internal_configusrstr = (char *) malloc(strlen("ffile=/adaqfs/halla/apar/g0inj/flags/g0inj.flags,badc=0xC1,nadc=3,vqwkinternal=0,vqwk_verbose=1" 
)+1); strcpy(internal_configusrstr,"ffile=/adaqfs/halla/apar/g0inj/flags/g0inj.flags,badc=0xC1,nadc=3,vqwkinternal=0,vqwk_verbose=1" 
);   }
...
if(( READOUT_VQWK == 1) ) {
    daLogMsg("INFO","Send a disable signal to the VQWK ADC gates during go");
{ 
  firstEvents = 2;
  set_tir_oport(0x40 );
 } 
...
16) | (( 0x10 ) << 8) | (0xff & *(rol->nevents));	}	((rol->dabufp))++;} ; if(( READOUT_BEAM_SCALERS == 1) ) {
...
( 0x01 ) << 8) | ( 0 );	((rol->dabufp))++; ; { 
	 int id = 0;
	   ...
	   set_tir_oport(0x10 );
           while (id < nvqwk){
             timer[id] = vqwkWaitForDataReady(id, 0xFF, 40);
             if (timer[id]<0) {
