...
int adc18_setup(int id, int time, int intgain, int conv) {
  ...
  adc18_init(id,0xed8000,0xed8100);  /* VME address of board */
...
/* I'm too tired to continue now ... but here are some notes on
  ...
  long csr; 
  int ramp, window, oversample, iinput;
...
/* ADC18: this is like defaultSetup, for external timing  */
  ...
  adc18_intgain(0,3);  
  adc18_setconv(0,0);
  adc18_setsample(0,1);  /* 2nd arg is 1,2,4, or 8 */
...
/* oversample=0 means no oversampling, `oversample N' means N+1 samples */  
  ...
  for (iev = 0; iev < 100000; iev++) {
    ...
    printf("(*********** NEW VERSION *************** Num of events %d   oldevnum %d    num words %d \n",nevt,oldevnum,nword);

    for (ievloop = 0; ievloop < oversample+1; ievloop++) {   /* hardcode oversample+1 for now */    
